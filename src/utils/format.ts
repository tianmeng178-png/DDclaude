...
import { getTimeZone } from './intl.js'
import { getUiLanguage, isChineseUi, t } from './i18n.js'
...
// `new Intl.NumberFormat` and `new Intl.RelativeTimeFormat` are expensive, so
...
const getNumberFormatter = (
  ...
  const key = `${locale}:${useConsistentDecimals ? 'consistent' : 'compact'}`
...
export function formatRelativeTime(
  ...
  const useChineseUi = isChineseUi()

  // Define time intervals with custom short units
  const intervals = [
    { unit: 'year', seconds: 31536000, shortUnit: useChineseUi ? '年' : 'y' },
    { unit: 'month', seconds: 2592000, shortUnit: useChineseUi ? '个月' : 'mo' },
    { unit: 'week', seconds: 604800, shortUnit: useChineseUi ? '周' : 'w' },
    { unit: 'day', seconds: 86400, shortUnit: useChineseUi ? '天' : 'd' },
    { unit: 'hour', seconds: 3600, shortUnit: useChineseUi ? '小时' : 'h' },
    { unit: 'minute', seconds: 60, shortUnit: useChineseUi ? '分钟' : 'm' },
    { unit: 'second', seconds: 1, shortUnit: useChineseUi ? '秒' : 's' },
  ] as const
  ...
  for (const { unit, seconds: intervalSeconds, shortUnit } of intervals) {
    if (Math.abs(diffInSeconds) >= intervalSeconds) {
      ...
      return getRelativeTimeFormatter('long', numeric).format(value, unit)
    }
  }
  ...
  return getRelativeTimeFormatter(style, numeric).format(0, 'second')
}
...
export function formatLogMetadata(log: {
  ...
  const sizeOrCount =
    log.fileSize !== undefined
      ...
      : t('common_messages_count', { count: log.messageCount })
  const parts = [
...
export function formatResetTime(
  ...
  if (hoursUntilReset > 24) {
    ...
    if (isChineseUi()) {
      return dateString + (showTimezone ? ` (${getTimeZone()})` : '')
    }
  ...
  const timeString = date.toLocaleTimeString(locale, {
    ...
    hour12: isChineseUi() ? false : true,
  })

  if (isChineseUi()) {
    return timeString + (showTimezone ? ` (${getTimeZone()})` : '')
  }
